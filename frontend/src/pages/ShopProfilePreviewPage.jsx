...
const ShopProfilePreviewPage = () => {
    ...
    const fetchShopOrderTypes = async () => {
      ...
      try {
        const response = await fetch(
          'http://localhost:5555/shop/shopOrderTypes',
          {
            method: 'GET',
            headers: {
  ...
  const handleSavePreviewImage = async () => {
    ...
    try {
      const response = await fetch(
        'http://localhost:5555/shop/shopdata/previewImage',
        {
          method: 'POST',
          headers: {
            'x-access-token': token,
  ...
  useEffect(() => {
    const fetchAllPreviewImage = async () => {
      ...
      try {
        const response = await fetch(
          'http://localhost:5555/shop/getAllpreviewImage',
          {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
  ...
  return (
    <div>
      <div className='flex pl-5 pt-5'>
        <div className='grid justify-start w-[calc((100vw-19rem))]'>
          {/* Shop Preview Image */}
          <div className='w-[500px] flex flex-col'>
            <div className='w-full h-[80px] flex justify-start items-center border-b'>
              <p className='text-3xl font-semibold'>Shop Preview Image</p>
            </div>
            <div className='w-full h-[300px] flex flex-col pt-3'>
              ...
              <div className='w-full h-5/6  flex'>
                ...
                ) : shopInfo?.previewImage &&
                  shopInfo.previewImage.length > 0 ? (
                  shopInfo.previewImage
                    .slice(0, 4)
                    .map((url, index) => (
                      <img
            ...
            <div className='w-full flex flex-col pt-3'>
              ...
              {orderTypes.map((orderType, index) => (
                <div className='pl-5 pt-5' key={index}>
                  <p className='mb-6 border-b text-xl'>{`Package ${
                    index + 1
                  }`}</p>
